	...
	public dynamicObject
{
	public:
		...
		//blockGroup
		void setBlock(int x, int y, int z, block &targetBlock);//用以进行小规模的方块替换。大规模的替换分为两类：其一为受到外界作用产生的大规模替换，这里用受力过程来进行操作。其二为生成世界时的大规模替换（创造），这里需要将其绑定至一个TerrainGenerator上。
	private:
